...
import { derivePurposeKey, encryptForPurpose, decryptForPurpose } from '../../../../../packages/providers/src/index'
...
export async function GET(request: NextRequest, context: Context) {
  ...
  if (path === 'admin/users') {
    ...
    values.push(limit + 1)
    const r = await db().query(`SELECT u.id,u.email,u.role,u.status,u.created_at FROM users u WHERE ${where.replaceAll('deleted_at', 'u.deleted_at').replaceAll('created_at', 'u.created_at').replaceAll('(u.created_at,id)', '(u.created_at,u.id)')} ORDER BY u.created_at DESC,u.id DESC LIMIT $${values.length}`, values)
...
export async function POST(request: NextRequest, context: Context) {
  ...
  if (path === 'generations') {
    ...
    const allowedGenerationFields = new Set(['prompt', 'modelId', 'parameters', 'inputs', 'idempotencyKey', 'inputLanguage', 'size', 'quality', 'count', 'inputImageIds'])
