-- Frozen copy of the statement stream executed by the historical
-- packages/database/src/migrate.ts, captured at the moment the sequential
-- migration system was introduced. Never edited. src/migrations/migrations.test.ts
-- asserts migrations/0001..0026 reproduce these statements exactly, so a
-- statement lost during the split fails CI without needing a database.
